...
# specific monitoring user
user_role contains "monitoring" if {
    user.email == "erik.belak@gmail.com"
}

# action is allowed if there is some overlap between request and user roles
action_allowed if {
    some role
...
# allow access if user is authenticated and action_allowed
allow if {
    user.valid
    action_allowed
}
